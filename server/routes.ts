...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // --- Core monitoring MVP — watched wallets, tokens, alerts ---

  const addWatchedWalletSchema = z.object({
    ownerPubkey: z.string().regex(SOLANA_ADDRESS_RE),
  ...
  app.get("/api/wallets", async (req, res) => {
    try {
      ...
      if (owner && !SOLANA_ADDRESS_RE.test(owner)) {
  ...
  app.get("/api/wallets/:id", async (req, res) => {
    try {
      const wallet = await storage.getWatchedWallet(req.params.id);
  ...
  app.delete("/api/wallets/:id", async (req, res) => {
    try {
      const removed = await storage.deleteWatchedWallet(req.params.id);
  ...
  app.get("/api/wallets/:id/tokens", async (req, res) => {
    try {
      const wallet = await storage.getWatchedWallet(req.params.id);
      if (!wallet) return res.status(404).json({ error: "Wallet not found" });
  ...
  app.get("/api/wallets/:id/alerts", async (req, res) => {
    try {
      const wallet = await storage.getWatchedWallet(req.params.id);
      if (!wallet) return res.status(404).json({ error: "Wallet not found" });
  ...
  // --- Remaining token routes (kept from original) ---
