  ...
  try {
    ...
    while (page <= maxPages) {
      ...
      for (const item of items) {
        ...
        if (!interfaceType.toLowerCase().includes("fungible") && item.token_info == null) {
          continue;
        }
        const mint = item.id || item.token_info?.mint;
        if (!mint) continue;
        const decimals = Number(item.token_info?.decimals ?? 0);
        const raw = Number(item.token_info?.balance ?? item.token_info?.amount ?? 0);
        const amount = decimals > 0 ? raw / Math.pow(10, decimals) : raw;
